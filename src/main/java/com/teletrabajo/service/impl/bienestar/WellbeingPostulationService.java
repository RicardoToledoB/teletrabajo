...
public class WellbeingPostulationService {
    ...
    private SummaryResponse buildSummary(WellbeingPostulationEntity p) {
        ...
        HousingRequest housing = housingRepository.findByPostulationId(p.getId()).map(h -> HousingRequest.builder().typeHousingId(h.getTypeHousing()!=null?h.getTypeHousing().getId():null).typePropertyId(h.getTypeProperty()!=null?h.getTypeProperty().getId():null).housingBackground(h.getHousingBackground()).otherBackground(h.getOtherBackground()).build()).orElse(null);
