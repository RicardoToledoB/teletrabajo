...
public class WellbeingPostulationService {
    ...
    public PostulationResponse updateMyAffiliate(Long id, AffiliateRequest request) {
        ...
        applyAffiliateRequest(p, request);
    ...
    public PostulationResponse updateAffiliate(Long id, AffiliateRequest request) {
        WellbeingPostulationEntity p = getEditablePostulation(id);
        applyAffiliateRequest(p, request);
        return mapPostulation(postulationRepository.save(p));
    }

    private void applyAffiliateRequest(WellbeingPostulationEntity p, AffiliateRequest request) {
        ...
        if (request.getStablishmentId() != null) {
            p.setStablishment(stablishmentRepository.findById(request.getStablishmentId())
    ...
    public SimpleResponse createIncome(Long postulationId, IncomeRequest request) {
        ...
        return SimpleResponse.builder().id(e.getId()).name(e.getIncomeType() != null ? e.getIncomeType().getName() : "Ingreso").amount(e.getAmount()).build();
    }
    ...
    private SummaryResponse buildSummary(WellbeingPostulationEntity p) {
        ...
        List<SimpleResponse> incomes = incomeRepository.findByPostulationIdOrderByIdAsc(p.getId()).stream().map(i -> SimpleResponse.builder().id(i.getId()).name(i.getIncomeType()!=null?i.getIncomeType().getName():"Ingreso").amount(i.getAmount()).build()).toList();
    ...
    private WellbeingPostulationEntity getPostulation(Long id) {
        WellbeingPostulationEntity p = postulationRepository.findAnyByIdIncludingDeleted(id)
                .orElseThrow(() -> new RuntimeException("Postulación no encontrada"));
    ...
    private WellbeingPostulationEntity getDraftPostulation(Long id) {
        return getEditablePostulation(id);
    }

    private WellbeingPostulationEntity getEditablePostulation(Long id) {
        WellbeingPostulationEntity p = postulationRepository.findAnyByIdIncludingDeleted(id)
                .orElseThrow(() -> new RuntimeException("Postulación no encontrada"));
    ...
    private void ensureNotDeleted(WellbeingPostulationEntity p) {
    ...
    private void ensureDraft(WellbeingPostulationEntity p) {
        if (p.getStatus() != BienestarEnums.PostulationStatus.DRAFT && p.getStatus() != BienestarEnums.PostulationStatus.OBSERVED) {
            throw new RuntimeException("La postulación no se puede modificar en estado " + p.getStatus());
        }
    }
    ...
    private SimpleResponse mapExpense(WellbeingFamilyExpenseEntity e){ return SimpleResponse.builder().id(e.getId()).code(e.getCode()).name(e.getName()).amount(e.getAmount()).build(); }
